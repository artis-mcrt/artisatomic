...
- `clear_files()` in `output.py` writes the file comment at the top of adata.txt, transitiondata.txt and phixsdata_v2.txt: the format of the file, and the options of the run that apply to all ions. The file comment must explain each field of the file to a reader who has only that file. Update it when a writer changes a field. A value that is the same for all ions belongs there and not in the comment block of each ion. The file comment gives the creation time in UTC. `creation_time_utc()` in `base.py` gives a time of zero in the test mode, and it obeys `SOURCE_DATE_EPOCH`, so the checksums stay the same. Do not put a version there, because the checksums would change with each commit.
- `write_comment_block()` in `output.py` writes the `#` comment block of an ion. ARTIS skips a comment line only directly before the header line of an ion (adata.txt, transitiondata.txt) or of a table (phixsdata_v2.txt). Write a comment at no other position. compositiondata.txt takes no comment.
- The run writes one log file for all ions, `artisatomiclog.txt`, and the record `artisatomicionhandlers.json`. Both are in the output folder, beside adata.txt. `log_path()` in `base.py` gives the path of the log file. The run empties it at its start, and each pass of each ion appends to it.
- A reader calls `log_comment()` from `base.py` for a summary line that belongs in a comment block. It calls `log_and_print()` for a detail line, and for a line that only repeats a number of the header line of the ion (the count of levels, the count of transitions, the ionisation energy). A comment line must not depend on the machine. Give each path through `path_for_log()`, with `relative_to` for a data folder that can be outside the repository.
