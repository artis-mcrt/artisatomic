...
- A reader calls `log_comment()` from `base.py` for a summary line that belongs in a comment block. It calls `log_and_print()` for a detail line. A comment line must not depend on the machine. Give each path through `path_for_log()`, with `relative_to` for a data folder that can be outside the repository.
