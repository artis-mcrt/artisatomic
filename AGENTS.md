...
- A reader calls `log_comment()` from `base.py` for a summary line that belongs in a comment block. It calls `log_and_print()` for a detail line. It also calls `log_and_print()` for a line that only repeats a number of the data header line of the ion. Examples are the count of levels, the count of transitions, and the ionisation energy.
