...
- The run writes one log file for all ions, `artisatomiclog.txt`, and the record `artisatomicionhandlers.json`. Both are in the output folder, beside adata.txt. If the output folder is the working directory, the record goes into the log file only, because `get_ion_handlers()` reads a file of that name there. `log_path()` in `base.py` gives the path of the log file. The run empties it at its start, and each pass of each ion appends to it.
