...
- A reader calls `log_comment()` from `base.py` for a summary line that belongs in a comment block. A line that can occur for many levels or transitions of one ion goes through `log_detail()`. The log file then gets each line, and the comment block gets the first line of each kind with the count. A line goes through `log_and_print()` only if it has no information for a user of the output files. It also calls `log_and_print()` for a line that only repeats a number of the data header line of the ion. Examples are the count of levels, the count of transitions, and the ionisation energy.
