...
- A reader calls `log_comment()` from `base.py` for a summary line that belongs in a comment block. It calls `log_and_print()` for a detail line. A comment line must not depend on the location of the checkout, so give each path through `path_for_log()`.
- The `description` of a `Handler` names the data source and its full reference. The comment blocks show it.
