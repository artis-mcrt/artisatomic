...
INTEGRALSTEPFACTOR = 5000 #5000 is good
...
def main():
    ...
    for e in range(len(listelements)):
      ...
      nahar_core_state_id_of_nahar_state = [{} for x in listions] #keys are (2S+1, L, parity, indexinsymmetry), values are integer id of Nahar core state
      nahar_configurations = [{} for x in listions] #keys are (2S+1, L, parity), values are strings of electron configuration
      nahar_phixs_tables = [{} for x in listions] #keys are (2S+1, L, parity, indexinsymmetry), values are lists of (energy in Rydberg, cross section in Mb) tuples
      nahar_ionization_potential_rydberg = [0.0 for x in listions]
      nahar_level_index_of_state = [{} for x in listions]
      
      hillier_energy_levels = [['IGNORE'] for x in listions] #list of named tuples (first element is IGNORE to discard 0th index)
      hillier_level_ids_matching_term = [{} for x in listions] #keys are tuples (2S+1, L, parity), item is a list of truncated level names in energy order
      hillier_ionization_energy_ev = [0.0 for x in listions]
      
      transitions = [['IGNORE'] for x in listions] #list of named tuples (hillier_transition_row)
      ...
      photoionization_crosssections = [[] for x in listions] #keys are hillier level ids, values are lists of (energy in Rydberg, cross section in Mb) tuples
      
      for i in range(len(listions)):
          (ionization_stage, path_hillier_osc_file, path_nahar_px_file, hillier_row_format_energy_level) = listions[i]
      ...
      with open(os.path.join(output_folder_artis,'adata.txt'), 'a') as fatommodels, \
        ...
        open(os.path.join(output_folder_transition_guide,'transitions_{}.txt'.format(elsymbols[atomic_number])), 'w') as ftransitionguide:
          ...
          ftransitionguide.write('{0:16s} {1:12s} {2:3s} {3:9s} {4:17s} {5:17s} {6:10s} {7:25s} {8:25s} {9:17s} {10:17s} {11:19s}\n'.format('lambda_angstroms','A','Z','ion_stage','lower_energy_Ev','lower_statweight','forbidden','lower_level','upper_level','upper_statweight','upper_energy_Ev','upper_has_permitted'))
          for i in range(len(listions)):
              (ionization_stage, path_hillier_osc_file, path_nahar_px_file, hillier_row_format_energy_level) = listions[i]
              ionstr = '{0} {1}'.format(elsymbols[atomic_number],roman_numerals[ionization_stage])
              flog = open(os.path.join(output_folder_artis,output_folder_logs,'{0}{1:d}.txt'.format(elsymbols[atomic_number].lower(),ionization_stage)), 'a')

              print('==============> ' + ionstr + ':')
              
              # write output files for artis
              logprint("writing to 'adata.txt'")
              fatommodels.write('{0:12d}{1:12d}{2:12d}{3:15.7f}\n'.format(atomic_number,ionization_stage,len(energy_levels[i])-1,hillier_ionization_energy_ev[i]))
              ...
              for levelid in range(1,len(energy_levels[i])):
                  ...
                  if hasattr(energylevel, 'hilliername') and energylevel.hilliername in transition_count_of_hillier_level_name[i]:
                      transitioncount = transition_count_of_hillier_level_name[i][energylevel.hilliername]
                  else:
                      transitioncount = 0
                  fatommodels.write('{0:7d}{1:25.16f}{2:25.16f}{3:7d}\n'.format(levelid,hc_in_ev_cm*float(energylevel.energyabovegsinpercm),float(energylevel.g),transitioncount))
              fatommodels.write('\n')

              logprint("writing to 'transitiondata.txt'")
              num_forbidden_transitions = 0
              ftransitiondata.write('{0:7d}{1:7d}{2:12d}\n'.format(atomic_number,ionization_stage,len(transitions[i])-1))
              ...
              for transitionid in range(1,len(transitions[i])):
                  ...
                  forbidden = (energy_levels[i][levelid_from].parity == energy_levels[i][levelid_to].parity)
                  if not forbidden:
                    level_ids_with_permitted_down_transitions.add(levelid_to) #hopefully 'to_level' is the upper level
              ...
              for transitionid in range(1,len(transitions[i])):
                  ...
                  if forbidden:
                      ...
                      coll_str = -2
                      flog.write('Forbidden transition: lambda_angstrom= {:7.1f}, {:25s} to {:25s}\n'.format(float(transition.lambdaangstrom),transition.namefrom,transition.nameto))
                  else:
                      coll_str = -1
                  ...
                  for lowerlevelid in range(1,len(energy_levels[i])):
                      ...
                      if core_state_id > 0 and core_state_id < len(nahar_core_states[i]):
                          ...
                          if core_state_id not in upper_level_ids_of_core_state_id.keys(): #go find matching levels if they haven't been found yet
                              ...
                              nahar_core_state_reduced_configuration = reduce_configuration(nahar_core_state.configuration + '_' + nahar_core_state.term)
                              ...
                              for upperlevelid in range(1,len(energy_levels[i+1])):
                                  ...
                                  if hasattr(upperlevel,'hilliername'):
                                      ...
                                      state_tuple = (int(upperlevel.twosplusone),int(upperlevel.l),int(upperlevel.parity),int(upperlevel.indexinsymmetry))
                                  ...
                                  if reduce_configuration(upperlevelconfig) == nahar_core_state_reduced_configuration:
                                      ...
                                      logprint("Matched core state {0} '{1}_{2}' to upper ion level {3} '{4}'".format(core_state_id,nahar_core_state.configuration,nahar_core_state.term,upperlevelid,upperlevelconfig))
                          
                              if len(upper_level_ids_of_core_state_id[core_state_id]) == 0:
                                  upper_level_ids_of_core_state_id[core_state_id] = [1]
                                  logprint("No upper level matched core state {0} '{1}_{2}' (reduced string: '{3}')".format(core_state_id,nahar_core_state.configuration,nahar_core_state.term,nahar_core_state_reduced_configuration))
                      ...
                      if upperionlevelids == [1]:
                          fphixs.write('{0:12d}{1:12d}{2:8d}{3:12d}{4:8d}\n'.format(atomic_number,ionization_stage+1,1,ionization_stage,lowerlevelid))
                      else:
                          fphixs.write('{0:12d}{1:12d}{2:8d}{3:12d}{4:8d}\n'.format(atomic_number,ionization_stage+1,-1,ionization_stage,lowerlevelid))
                          fphixs.write('{0:8d}\n'.format(len(upperionlevelids)))
                          
                          summed_statistical_weights = sum([float(energy_levels[i+1][id].g) for id in upperionlevelids])
                          for upperionlevelid in sorted(upperionlevelids):
                              phixsprobability = float(energy_levels[i+1][upperionlevelid].g) / summed_statistical_weights
                              fphixs.write('{0:8d}{1:12f}\n'.format(upperionlevelid,phixsprobability))
...
def read_nahar_energy_level_file(path_nahar_energy_file):
    nahar_energy_level_row = collections.namedtuple('energylevel','indexinsymmetry TC corestateid elecn elecl energyreltoionpotrydberg twosplusone l parity energyabovegsinpercm g')
    naharcorestaterow = collections.namedtuple('naharcorestate','nahar_core_state_id configuration term energyrydberg')
    with open(path_nahar_energy_file, 'r') as fenlist:
        ...
        for c in range(1,numberofcorestates+1):
            ...
            nahar_core_states[i][c] = naharcorestaterow._make((int(row[0]),row[1],row[2],float(row[3])))
        ...
        while True:
            ...
            if line.startswith(' Ion ground state'):
                ...
                flog.write('Ionization potential = {0} Ryd\n'.format(nahar_ionization_potential_rydberg[i]))
        ...
        if atomic_number != int(row[0]) or ionization_stage != int(row[0]) - int(row[1]):
                print('Wrong atomic number or ionization stage in Nahar energy file',atomic_number,int(row[0]),ionization_stage,int(row[0]) - int(row[1]))
                sys.exit()
        ...
        while True:
            ...
            for s in range(number_of_states_in_symmetry):
                ...
                if nahar_core_state_id < 1 or nahar_core_state_id > len(nahar_core_states[i]):
                    flog.write("Core state id of {0:d}{1}{2} index {3:d} is invalid (={4:d}, Ncorestates={5:d})\n".format(twosplusone,lchars[l],['e','o'][parity],indexinsymmetry,nahar_core_state_id,len(nahar_core_states[i])))
                    if nahar_core_state_id == 0:
                      flog.write("(setting the core state to 1 instead)\n")
                      nahar_core_state_id = 1
                
                nahar_energy_levels[i].append(nahar_energy_level_row._make(row+[twosplusone,l,parity,-1.0,0]))
                energyabovegsinpercm = (nahar_ionization_potential_rydberg[i] + float(nahar_energy_levels[i][-1].energyreltoionpotrydberg)) * ryd_to_ev / hc_in_ev_cm

                nahar_energy_levels[i][-1] = nahar_energy_levels[i][-1]._replace(
                    indexinsymmetry = indexinsymmetry,
                    corestateid = nahar_core_state_id,
                    energyreltoionpotrydberg = float(nahar_energy_levels[i][-1].energyreltoionpotrydberg),
                    energyabovegsinpercm = energyabovegsinpercm,
                    g = twosplusone * (2*l +1)
                )
                nahar_level_index_of_state[i][(twosplusone,l,parity,indexinsymmetry)] = len(nahar_energy_levels[i])-1
...
def read_hillier_levels_and_transitions_file(hillier_ion_folder, path_hillier_osc_file, hillier_row_format_energy_level):
    hillier_energy_level_row = collections.namedtuple('energylevel',hillier_row_format_energy_level + ' corestateid twosplusone l parity indexinsymmetry')
    hillier_transition_row = collections.namedtuple('transition','namefrom nameto f A lambdaangstrom i j hilliertransitionid')
                ...
                if twosplusone == -1: #-1 would mean that the term could not be interpreted
                    ...
                    if (twosplusone,l,parity) in hillier_level_ids_matching_term[i].keys():
                        if levelname not in hillier_level_ids_matching_term[i][(twosplusone,l,parity)]:
                            hillier_level_ids_matching_term[i][(twosplusone,l,parity)].append(hillierlevelid)
                    else:
                        hillier_level_ids_matching_term[i][(twosplusone,l,parity)] = [hillierlevelid,]
                
                if float(hillier_energy_levels[i][-1].energyabovegsinpercm) < 1.0: #if this is the ground state
                    hillier_ionization_energy_ev[i] = hc_in_ev_angstrom / float(hillier_energy_levels[i][-1].lambdaangstrom)
...
#                    defined_transition_ids.append(int(transition.hilliertransitionid))
                    ...
                    if int(transition.hilliertransitionid) != len(transitions[i])-1:
                        print(hillier_ion_folder + path_hillier_osc_file + ', WARNING: Transition id {0:d} found at entry number {1:d}'.format(int(transition.hilliertransitionid),len(transitions[i])-1))
#                    sys.exit()
                else:
                    logprint('FATAL: multiply-defined Hillier transition: {0} {1}'.format(transition.namefrom,transition.nameto))
                    sys.exit()
...
def read_nahar_phixs_tables(path_nahar_px_file):
    with open(path_nahar_px_file, 'r') as fenlist:
        ...
        row = line.split()
        if atomic_number != int(row[0]) or ionization_stage != int(row[0]) - int(row[1]):
             print('Wrong atomic number or ionization stage in Nahar file',atomic_number,int(row[0]),ionization_stage,int(row[0]) - int(row[1]))
             sys.exit()
        ...
        while True:
            ...
            nahar_phixs_tables[i][(twosplusone,l,parity,indexinsymmetry)] = np.empty((number_of_points,2))
            for p in range(number_of_points):
                row = fenlist.readline().split()
                nahar_phixs_tables[i][(twosplusone,l,parity,indexinsymmetry)][p] = (float(row[0]),float(row[1]))
...
def combine_sources():
    ...
    added_nahar_levels = []
    #match up Nahar states given in phixs data with Hillier levels, adding missing levels as necessary
    for (twosplusone,l,parity,indexinsymmetry) in nahar_phixs_tables[i].keys():
        ...
        if state_tuple in nahar_configurations[i]:
          ...
          if nahar_configuration_this_state.strip() in nahar_configuration_replacements.keys():
            nahar_configuration_this_state = nahar_configuration_replacements[nahar_configurations[i][state_tuple].strip()]
            logprint("Replacing Nahar configuration of '{0}' with '{1}'".format(nahar_configurations[i][state_tuple],nahar_configuration_this_state))
            ...
            if nahar_configuration_this_state != '_CONFIG NOT FOUND_':
                for levelid in hillier_level_ids_matching_term[i][(twosplusone,l,parity)]:
                    ...
                    if reduce_configuration(levelname) == reduce_configuration(nahar_configuration_this_state) and \
                        ...
                        core_state_id = nahar_energy_levels[i][nahar_level_index_of_state[i][state_tuple]].corestateid
                        hillier_energy_levels[i][levelid] = hillier_energy_levels[i][levelid]._replace(twosplusone = twosplusone, l = l, parity = parity, indexinsymmetry = indexinsymmetry, corestateid = core_state_id)
                        hillier_level_ids_matching_this_nahar_state.append(levelid)
            else:
                logprint("No electron configuration for {0:d}{1}{2} index {3:d}".format(twosplusone,lchars[l],['e','o'][parity],indexinsymmetry))
        else:
            flog.write("No Hillier levels with term {0:d}{1}{2}\n".format(twosplusone,lchars[l],['e','o'][parity],indexinsymmetry))
        ...
        if len(hillier_level_ids_matching_this_nahar_state) == 0:
            ...
            if state_tuple in nahar_level_index_of_state[i]:
                nahar_energy_level = nahar_energy_levels[i][nahar_level_index_of_state[i][state_tuple]]
            ...
            #HERE
            nahar_energyabovegsinev = hc_in_ev_cm * nahar_energy_levels[i][nahar_level_index_of_state[i][state_tuple]].energyabovegsinpercm
            avghillierthreshold = weightedavgthresholdinev(hillier_energy_levels[i],hillier_level_ids_matching_this_nahar_state)
            ...
            avghillierenergyabovegsinev = weightedavgenergyinev(hillier_energy_levels[i],hillier_level_ids_matching_this_nahar_state)
            strhilliermatchesenergy = '[' + ', '.join(['{0} ({1:.3f} eV)'.format(hillier_energy_levels[i][k].hilliername,hc_in_ev_cm*float(hillier_energy_levels[i][k].energyabovegsinpercm)) for k in hillier_level_ids_matching_this_nahar_state]) + ']'
    ...
    logprint('Included {0} levels from Hillier dataset and added {1} levels from Nahar phixs tables for a total of {2} levels'.format(len(hillier_energy_levels[i])-1,len(added_nahar_levels),len(energy_levels[i])-1))

    #now sort the updated energy level list by energy
    energy_levels[i].sort(key=lambda x: float(x.energyabovegsinpercm) if hasattr(x,'energyabovegsinpercm') else float('-inf'))

    #generate a mapping of level names to the final output level ids
    for levelid in range(1,len(energy_levels[i])):
    ...
    for (twosplusone,l,parity,indexinsymmetry) in nahar_phixs_tables[i].keys():
        ...
        reduced_phixs_list = reduce_phixs_table(nahar_phixs_tables[i][(twosplusone,l,parity,indexinsymmetry)])
        foundamatch = False
        for levelid in range(1,len(energy_levels[i])):
        ...
        if not foundamatch:
            logprint("No Hiller or Nahar state to match with photoionization crosssection of {0:d}{1}{2} index {3:d}".format(twosplusone,lchars[l],['e','o'][parity],indexinsymmetry))
    return
...
#this method downsamples the photoionization cross section table to a regular grid while keeping the recombination rate integral constant (assuming that the temperature matches)
...
#    xgrid = list(filter(lambda x:x < (listin[-1][0]/listin[0][0]),xgrid))
    ...
    for i in range(len(xgrid)-1):
        ...
        listenergyryd = np.linspace(enlow,enhigh,num=NPHIXSNUINCREMENT*INTEGRALSTEPFACTOR,endpoint=False) #change back to *5000
        #dnu = (listenergyryd[1] - listenergyryd[0]) * ryd_to_erg / h_in_erg_seconds

        listsigma_bf_megabarns = np.interp(listenergyryd,listin[:,:1].flatten(),listin[:,1:].flatten(),right=-1)
...
#        listsigma_bf_megabarns = np.interp(listenergyryd,*zip(*listin),right=-1)
        ...
        for j in range(1,len(listenergyryd)):
            ...
            if sigma_bf_megabarns < 0: #negative value means we're in the extrapolation regime
                sigma_bf_megabarns = (listin[-1][0]/energyryd) ** 3 * listin[-1][1] #assume a power law decline from the highest-energy cross section point
            nu = energyryd * ryd_to_erg / h_in_erg_seconds
            
            integrandnosigma = (nu ** 2) * math.exp(-h_over_kb_in_kelvin_seconds*(nu-nu0)/OPTIMALTEMPERATURE)
...
def weightedavgenergyinev(energy_levelsthision,ids):
    ...
    gsum = 0.0
    for id in ids:
        statisticalweight = float(energy_levelsthision[id].g)
        sum += statisticalweight * hc_in_ev_cm * float(energy_levelsthision[id].energyabovegsinpercm)
...
def weightedavgthresholdinev(energy_levelsthision,ids):
    ...
    for id in ids:
        statisticalweight = float(energy_levelsthision[id].g)
        sum += statisticalweight * hc_in_ev_angstrom / float(energy_levelsthision[id].lambdaangstrom)
        gsum += statisticalweight
    return sum / gsum
...
def get_term_as_tuple(config):
    ...
    for i in reversed(range(len(config))):
        if config[i] in lchars:
            lposition = i
            l = lchars.index(config[i])
            break
...
#similar to Hillier style "3d6(5D)4s8p_5Fo" but without the parent term (and mysterious letter before the term if present)
