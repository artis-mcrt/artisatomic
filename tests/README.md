...
| `mons` | Ce V–VI | The MONS lanthanide reader, from the committed `test_sample/`. `atomic-data-mons/make_test_sample.py` cut it from the 21.7 GB archive: the lowest 450 levels of Ce V and 400 of Ce VI, plus every transition between two of them. Two ions, so Ce V is not the top ion. The level names carry no configuration, so artisatomic writes no hydrogenic phixs estimate. |
