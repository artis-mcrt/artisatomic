...
| `mons` | Ce V–VI | The MONS lanthanide reader, from the committed `test_sample/`: the lowest 450 levels of Ce V and 400 of Ce VI, with the transitions between them, cut from the 21.7 GB archive. Two ions, so Ce V is not the top ion. The level names carry no configuration, so no hydrogenic phixs estimate is written. |
