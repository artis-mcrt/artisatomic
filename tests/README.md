...
prefer adding it to the committed sample over introducing another download: the published Floers+25
corpus is 5.5 GiB, but `testdata.tar.xz` carries only the handful of ions the tests name.
...
| `floers25` | La III (uncalibrated), Yb II–III (calibrated) | Both Floers+25 branches: the reader takes a different pair of filenames for each, and `extend_ion_list()` offers both handlers. **Yb II** is deliberately not a top ion, so the floers25 level names are also exercised through the hydrogenic phixs estimate. These are the smallest ions that cover both branches — 176 KB of `testdata.tar.xz`, against 73 MB of transitions for Dy III alone. |
