...
| `qub` | Co II–IV, Sr I | The `qub_cobalt` handler, from the committed `co_tyndall_test_sample/`. Co II comes from CMFGEN, Co III–IV from the QUB adf04 files. **Sr I** uses the `qub_data` handler on the committed `38_1.adf04.zst`. That file is complete, so it carries the `-1` row that ends the collision block, and the Co sample stops before that row. It is also the only committed adf04 file that is compressed. Sr I is the top ion, so it writes no phixs block. Every one of its 1596 output transitions carries a collision strength, and 224 of them come from an upsilon value with no A-value. |
