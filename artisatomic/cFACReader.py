...
def GetLines(filename, Z, ion_name=0, date=date.today(), Get_csv=True, Get_dat=True):
    ...
    if Get_csv:
        with open('{}.csv'.format(filename), 'a') as f:
            ...
            f.write('***********************************************************************\n')
            f.write('Last updated: {} \n'.format(LastUpdated))
            f.write('Multipole:{}  #negative values for electric transitions, positive for magnetic, 1 - dipole, 2 - quadrupole ...\n'.format(Multi))
            f.write('Z: {}\n'.format(Z))
            f.write('Ionization Stage: {}\n'.format(2))
    ...
    if Get_dat:
        T = tabulate(lines, showindex=False, headers='keys', tablefmt='plain',
...
def GetOpacities(dfE, dfT, M, ion_name, T=10000, z=1, rho=1e-13, t=86400, wbins=10, minbin=0, maxbin=25001, ToFeather=False, ToCSV=False):
